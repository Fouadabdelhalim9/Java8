...
//        Consumer<Author> activeAuthorPrintConsumer = author -> {
//            if(author.active) {
//                System.out.println(author);
//            } };
//        authors.forEach(activeAuthorPrintConsumer);
...
//        banner("Active books for all authors");
        // TODO With functional interfaces declared
//        Consumer<Author> activeBooksPrintConsumer = author -> {
//            System.out.println("Active books for " + author.name);
//            author.books.stream()
//                    .filter(Book::isPublished)
//                    .forEach(System.out::println);
//        };
//        authors.stream().forEach(activeBooksPrintConsumer);
        banner("Active books for all authors - lambda");
        // TODO With functional interfaces used directly
//        authors.stream()
//                .flatMap(author -> author.books.stream().filter(Book::isPublished))
//                .forEach(System.out::println);
...
//        System.out.println("Average price: $" + averagePrice);
        ...
        // TODO With functional interfaces used directly
//        double totalPriceLambda = authors.stream()
//                .flatMap(author -> author.books.stream())
//                .mapToInt(book -> book.price)
//                .sum();
//        int totalBooksLambda = (int) authors.stream().flatMap(author -> author.books.stream()).count();
//        double averagePriceLambda = totalBooksLambda > 0 ? totalPriceLambda / totalBooksLambda : 0;
//        System.out.println("Average price: $" + averagePriceLambda);

        banner("Active authors that have at least one published book");
        // TODO With functional interfaces declared
//        authors.stream()
//                .filter(author -> author.active && author.books.stream().anyMatch(Book::isPublished))
//                .forEach(System.out::println);
